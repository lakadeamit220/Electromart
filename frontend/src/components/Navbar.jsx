  ...
  return (
    <nav className="bg-gradient-to-r from-blue-600 to-blue-800 p-4 shadow-lg">
      <div className="container mx-auto flex justify-between items-center">
        <Link to="/" className="text-white text-3xl font-extrabold tracking-tight">ElectroMart</Link>
        <div className="flex items-center space-x-6">
          <Link to="/" className="text-white hover:text-blue-200 transition">Home</Link>
          <Link to="/cart" className="text-white hover:text-blue-200 transition relative">
            Cart
            {cart.length > 0 && (
          ...
          {user ? (
            <>
              <Link to="/profile" className="text-white hover:text-blue-200 transition">Profile</Link>
              {user.isAdmin && (
                <Link to="/admin" className="text-white hover:text-blue-200 transition">Admin</Link>
              )}
              <button onClick={logout} className="text-white hover:text-blue-200 transition">Logout</button>
            </>
          ) : (
            <>
